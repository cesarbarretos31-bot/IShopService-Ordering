...
public class OrderRepository(IMongoDatabase database, IConfiguration configuration) : IOrderRepository
{
    ...
    public async Task<IReadOnlyList<Order>> GetAllAsync(CancellationToken ct) => await _orders.Find(Builders<Order>.Filter.Empty).SortByDescending(x => x.CreatedAt).ToListAsync(ct);
